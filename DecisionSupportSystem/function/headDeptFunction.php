...
function getCountStudentGradeRangeByCourseNameAndSemesterYear($courseName, $semesterYear)
{
    ...
    FROM studentstatus NATURAL JOIN gpastatus NATURAL JOIN fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student  INNER JOIN course ON fact_student.courseId = course.courseId
    WHERE status = 'กำลังศึกษา' AND nameCourseUse = '$courseName' AND termSummaryId IN (SELECT MAX(termSummaryId) AS termSummaryId FROM fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student  INNER JOIN course ON fact_student.courseId = course.courseId WHERE nameCourseUse = '$courseName' AND semesterYear <= $semesterYear   GROUP BY studentId);";
...
function geStudentListInGradeRangeByCourseNameAndSemesterYearAndGradeRange($courseName, $semesterYear, $gradeRange)
{
    ...
    FROM studentstatus NATURAL JOIN gpastatus NATURAL JOIN fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student NATURAL JOIN student INNER JOIN course ON fact_student.courseId = course.courseId
    WHERE status = 'กำลังศึกษา' AND nameCourseUse = '$courseName' AND gpaStatusName = '$gradeRange' AND termSummaryId IN (SELECT MAX(termSummaryId) AS  termSummaryId FROM gpastatus NATURAL JOIN fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student  INNER JOIN course ON fact_student.courseId = course.courseId WHERE nameCourseUse = '$courseName' AND semesterYear <= $semesterYear
    GROUP BY studentId);";
...
function getCountStudentTcasSortByTcasRoundByCourseName($courseName)
{
    ...
    $sql = "SELECT tcasRound,COUNT(CASE WHEN studyGeneretion = couseStartYear-2500 THEN studentId END) AS generetion1,COUNT(CASE WHEN studyGeneretion = couseStartYear-2500+1 THEN studentId END) AS generetion2,COUNT(CASE WHEN studyGeneretion = couseStartYear-2500+2 THEN studentId END) AS generetion3,COUNT(CASE WHEN studyGeneretion = couseStartYear-2500+3 THEN studentId END) AS generetion4,COUNT(CASE WHEN studyGeneretion = couseStartYear-2500+4 THEN studentId END) AS generetion5
    FROM course NATURAL JOIN fact_student NATURAL JOIN tcas
    WHERE nameCourseUse = '$courseName'
    GROUP BY tcasRound;";
...
function getCountStudentTcasSortByTcasRoundByCourseNameAndGeneretion($courseName, $generetion)
{
    ...
    $sql = "SELECT tcasRound,COUNT(CASE WHEN studyGeneretion = $generetion THEN studentId END) AS generetion1
    FROM course NATURAL JOIN fact_student NATURAL JOIN tcas
    WHERE nameCourseUse = '$courseName' AND studyGeneretion = $generetion
    GROUP BY tcasRound;";
...
function getCountStudentTcasSortByStudyGeneretionByCourseNameAndTcasRound($courseName, $round)
{
    ...
    $sql = "SELECT studyGeneretion,COUNT(studentId) AS tcasCount
    FROM course NATURAL JOIN fact_student NATURAL JOIN tcas
...
function getMaxMinAvgGPAXSortByTcasRoundByCourseName($courseName)
{
    ...
    $sql = "SELECT tcasRound,ROUND(MAX(gpaAll),2) AS maxGPAX,ROUND(AVG(gpaAll),2) AS avgGPAX,ROUND(MIN(gpaAll),2) AS minGPAX
    FROM semester NATURAL JOIN fact_term_summary NATURAL JOIN tcas NATURAL JOIN fact_student  INNER JOIN course ON fact_student.courseId = course.courseId
    WHERE nameCourseUse = '$courseName' AND termSummaryId IN (SELECT MAX(termSummaryId) AS termSummaryId FROM fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student  INNER JOIN course ON fact_student.courseId = course.courseId WHERE nameCourseUse = '$courseName' GROUP BY studentId)
    GROUP BY tcasRound
    ORDER BY tcasRound;";
...
function getMaxMinAvgGPAXByCourseNameAndGeneretion($courseName, $generetion)
{
...
function getPercentageStudySortByTcasRoundByCourseName($courseName)
{
    ...
    $sql = "SELECT tcasRound,COUNT(studentId) AS entry,COUNT(CASE WHEN status = 'กำลังศึกษา' THEN studentId END) AS study,ROUND(COUNT(CASE WHEN status = 'กำลังศึกษา' THEN studentId END)*100/COUNT(studentId),2) AS percentage
    FROM semester NATURAL JOIN fact_term_summary NATURAL JOIN tcas NATURAL JOIN fact_student NATURAL JOIN studentstatus  INNER JOIN course ON fact_student.courseId = course.courseId
...
function getPercentageStudySortByTcasRoundByCourseNameAnGeereiond($courseName, $generetion)
{
...
function getPercentageStudySortByGeneretionByCourseNameAndRound($courseName, $round)
{
    ...
    FROM semester NATURAL JOIN fact_term_summary NATURAL JOIN tcas NATURAL JOIN fact_student NATURAL JOIN studentstatus  INNER JOIN course ON fact_student.courseId = course.courseId
    WHERE nameCourseUse = '$courseName' AND termSummaryId IN (SELECT MAX(termSummaryId) AS termSummaryId FROM fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student NATURAL JOIN tcas INNER JOIN course ON fact_student.courseId = course.courseId 
    WHERE nameCourseUse = '$courseName' AND tcasRound = $round AND studyGeneretion <= $semesterYear-2500 AND studyGeneretion >= $semesterYear - 2503
    GROUP BY studentId)
    GROUP BY studyGeneretion;";
...
function getPercentageStudyAndRetireSortByTcasRoundByCourseName($courseName)
{
...
function getPercentageStudyAndRetireSortByTcasRoundByCourseNameAndGeneretion($courseName, $generetion)
{
...
function getCountStudentGradeRangeByDepartmrntIdAndSemesterYear($departmentId, $semesterYear)
{
    ...
    $sql = "SELECT COUNT(CASE WHEN gpaStatusName = 'blue' THEN studentId END) AS blue,COUNT(CASE WHEN gpaStatusName = 'green' THEN studentId END) AS green,COUNT(CASE WHEN gpaStatusName = 'orange' THEN studentId END) AS orange,COUNT(CASE WHEN gpaStatusName = 'red' THEN studentId END) AS red
    FROM studentstatus NATURAL JOIN gpastatus NATURAL JOIN fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student
    WHERE status = 'กำลังศึกษา' AND departmentId = $departmentId AND termSummaryId IN (SELECT MAX(termSummaryId) AS termSummaryId FROM fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student   WHERE departmentId = $departmentId AND semesterYear <= $semesterYear   GROUP BY studentId);";
...
function geStudentListInGradeRangeByDepartmentIdAndSemesterYearAndGradeRange($departmentId, $semesterYear, $gradeRange)
{
    ...
    $students = [];
    $sql = "SELECT studentId,fisrtNameTh,lastNameTh,round(gpaTerm,2) AS gpaTerm,round(gpaAll,2) AS gpaAll
    FROM studentstatus NATURAL JOIN gpastatus NATURAL JOIN fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student NATURAL JOIN student
    WHERE status = 'กำลังศึกษา' AND departmentId = $departmentId AND gpaStatusName = '$gradeRange' AND termSummaryId IN (SELECT MAX(termSummaryId) AS  termSummaryId FROM gpastatus NATURAL JOIN fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student 
    WHERE  departmentId = $departmentId AND semesterYear <= $semesterYear
    GROUP BY studentId);";
...
function getCountStudentTcasSortByTcasRoundByDepartmentId($departmentId)
{
    ...
    $sql = "SELECT tcasRound,COUNT(CASE WHEN studyGeneretion = $semesterYear-4-2500 THEN studentId END) AS generetion1,COUNT(CASE WHEN studyGeneretion = $semesterYear-3-2500 THEN studentId END) AS generetion2,COUNT(CASE WHEN studyGeneretion = $semesterYear-2-2500 THEN studentId END) AS generetion3,COUNT(CASE WHEN studyGeneretion = $semesterYear-1-2500 THEN studentId END) AS generetion4,COUNT(CASE WHEN studyGeneretion = $semesterYear-2500 THEN studentId END) AS generetion5
    FROM department NATURAL JOIN fact_student NATURAL JOIN tcas
    WHERE departmentId = $departmentId
    GROUP BY tcasRound
    ORDER BY tcasRound;";
...
function getCountStudentTcasSortByTcasRoundByDepartmentIdAndGeneretion($departmentId, $generetion)
{
...
function getMaxMinAvgGPAXSortByRoundByDepartmentId($departmentId)
{
...
function getMaxMinAvgGPAXSortByRoundByDepartmentIdAndGeneretion($departmentId, $generetion)
{
    ...
    $sql = "SELECT tcasRound,ROUND(MAX(gpaAll),2) AS maxGPAX,ROUND(AVG(gpaAll),2) AS avgGPAX,ROUND(MIN(gpaAll),2) AS minGPAX
    FROM semester NATURAL JOIN fact_term_summary NATURAL JOIN tcas NATURAL JOIN fact_student
    WHERE departmentId = $departmentId AND studyGeneretion = $generetion AND termSummaryId IN (SELECT MAX(termSummaryId) AS termSummaryId FROM fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student WHERE departmentId = $departmentId GROUP BY studentId)
    GROUP BY tcasRound
    ORDER BY tcasRound;";
...
function getPercentageStudySortByTcasRoundByDepartmentId($departmentId)
{
...
function getPercentageStudySortByTcasRoundByDepartmentIdAndGeneretion($departmentId, $generetion)
{
...
function getPercentageStudyAndRetireSortByRoundByDepartmentId($departmentId)
{
...
function getPercentageStudyAndRetireSortByRoundByDepartmentIdAndGeneretion($departmentId, $generetion)
{
...
function getGradeRangeSortByAdviserByDepartmentIdAndSemesterYear($departmentId, $semesterYear)
{
    ...
    FROM studentstatus NATURAL JOIN gpastatus NATURAL JOIN fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student  NATURAL JOIN teacher
    WHERE status = 'กำลังศึกษา' AND departmentId = $departmentId AND termSummaryId IN (SELECT MAX(termSummaryId) AS termSummaryId FROM fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student NATURAL JOIN teacher
    WHERE departmentId = $departmentId AND semesterYear <= $semesterYear   GROUP BY studentId)
    GROUP BY teacherId;";
...
function getListStudentByTeacherIdAndGPAStatusNameAndSemesterYear($teacherId, $gpaStatusName, $semesterYear)
{
    ...
    $sql = "SELECT *
    FROM studentstatus NATURAL JOIN gpastatus NATURAL JOIN fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student  NATURAL JOIN teacher INNER JOIN student ON fact_student.studentId = student.studentId
    WHERE status = 'กำลังศึกษา' AND teacherId = $teacherId AND gpaStatusName = '$gpaStatusName' AND termSummaryId IN (SELECT MAX(termSummaryId) AS termSummaryId FROM fact_term_summary NATURAL JOIN semester NATURAL JOIN fact_student NATURAL JOIN teacher
